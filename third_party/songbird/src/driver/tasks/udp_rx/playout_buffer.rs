...
pub struct StoredPacket {
    ...
    pub decrypted: bool,
    // PATCH(voicechat-discord): carry the true payload tail computed during
    // decryption (crypto suffix + RTP padding + the DAVE E2EE trailer) so that
    // `get_voice_tick` does not have to recompute it from the generic
    // crypto-mode suffix length, which leaves trailing garbage in the opus
    // payload of DAVE-encrypted frames.
    /// Number of bytes at the end of `rtp.payload()` to discard.
    pub payload_tail: usize,
}
