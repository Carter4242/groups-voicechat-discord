    ...
    pub fn get_voice_tick(&mut self, config: &Config) -> Result<Option<VoiceData>> {
        ...
        let pkt = match m_pkt {
            PacketLookup::Packet(StoredPacket {
                ...
                decrypted,
                payload_tail,
            }) => Some((packet, decrypted, payload_tail)),
        ...
        if let Some((packet, decrypted, stored_tail)) = pkt {
            ...
            // PATCH(voicechat-discord): use the tail recorded at decryption
            // time. The generic crypto-mode suffix previously used here does
            // not account for RTP padding or the DAVE E2EE trailer, so
            // DAVE-encrypted frames carried trailing garbage in their opus
            // payload (breaking downstream decoders). The offset is unchanged:
            // the slice must still start at the extension preamble, which
            // `scan_and_decode` parses and skips itself.
            let payload_end_pad = payload.len() - stored_tail;
